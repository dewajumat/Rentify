                ...
                <div class="card-body">
                    <div class="row d-flex align-items-center">
                        <div class="col-lg-6 col-sm-12 image-container my-auto mx-auto">
                            <img src="{{ asset('image/' . basename($detailPembayaran->bukti_pembayaran)) }}" alt="">
                        ...
                        <div class="col">
                            <div class="row mt-3 mb-2">
                                ...
                                <div class="col text-start detM2">
                                    <p>:&ensp;{{ $detailPembayaran->name }}</p>
                                    <p>:&ensp;{{ $detailPembayaran->jenis_properti }}</p>
                                    @php
                                        $words = explode(' ', $detailPembayaran->alamat);
                                        $alamat = implode(' ', array_slice($words, 0, 3));
                                    @endphp
                                    <p>:&ensp;{{ $alamat }}</p>
                                    <p>:&ensp;{{ $detailPembayaran->no_bilik }}</p>
                                    <p>:&ensp;{{ $detailPembayaran->tipe_hunian }}</p>
                                    <p>:&ensp;{{ Carbon::parse($detailPembayaran->tgl_pembayaran)->format('d F Y') }}</p>
                                    <p>:&ensp;{{ Carbon::parse($detailPembayaran->bulan_start_terbayar)->format('d M y') }}
                                        -
                                        {{ Carbon::parse($detailPembayaran->bulan_end_terbayar)->format('d M y') }}</p>
                                    <p>:&ensp;@currency($detailPembayaran->total_pembayaran)</p>
                                </div>
                            </div>
                            <div>
